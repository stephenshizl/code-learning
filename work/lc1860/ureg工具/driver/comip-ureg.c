...
static int do_normal_reg_read(struct ureg_unit *n_reg)
{
	...
	if ((n_reg->paddr < ISP_BASE) || (n_reg->paddr > (ISP_BASE + 0x80000 - 1))){
		...
		printk("youyoudebug ISP REG READ,  [0x%08X, 0x%02X]\n", n_reg->paddr, n_reg->val);
	}
	else {
...
static int do_normal_reg_write(struct ureg_unit *n_reg)
{
	...
	if ((n_reg->paddr < ISP_BASE) || (n_reg->paddr > (ISP_BASE + 0x80000 - 1))){
		...
		printk("youyoudebug ISP REG WRITE, [0x%08X, 0x%02X]\n", n_reg->paddr, n_reg->val);
	}
	else {
		n_reg->paddr |= ISP_BASE;
...
static inline int do_i2c_read(struct ureg_i2c *n_i2c)
{
	...
	pr_err("youyoudebug do_i2c_read n_i2c->flag:0x%x\n",n_i2c->flag);
...
static inline int do_i2c_write(struct ureg_i2c *n_i2c)
{
	...
	pr_err("youyoudebug do_i2c_write n_i2c->flag:0x%x\n",n_i2c->flag);
	if(n_i2c->flag & UREG_I2C_ADDR_8BIT)
		byte_cnt = 1;
	else if(n_i2c->flag & UREG_I2C_ADDR_16BIT)
		byte_cnt = 2;
				...
				unsigned int cmd, unsigned long arg)
{
	...
	pr_err("youyoudebug ureg_ioctl cmd:0x%x\n",cmd);
	switch (cmd) {
	case UREG_READ_REG:
		if (copy_from_user(&n_reg, argp, sizeof(n_reg)))
			return -EFAULT;

		pr_err("youyoudebug UREG_READ_REG == n_reg.paddr=0x%x,n_reg.vaddr=0x%x,n_reg.val=0x%x\n");
	...
	case UREG_WRITE_REG:
		if (copy_from_user(&n_reg, argp, sizeof(n_reg)))
			return -EFAULT;
		pr_err("youyoudebug UREG_WRITE_REG == n_reg.paddr=0x%x,n_reg.vaddr=0x%x,n_reg.val=0x%x\n");
	...
	case UREG_READ_PMIC:
		...
		pr_err("youyoudebug UREG_READ_PMIC == n_reg.paddr=0x%x,n_reg.vaddr=0x%x,n_reg.val=0x%x\n");
	...
	case UREG_WRITE_PMIC:
		if (copy_from_user(&n_reg, argp, sizeof(n_reg)))
			return -EFAULT;
		pr_err("youyoudebug UREG_WRITE_PMIC == n_reg.paddr=0x%x,n_reg.vaddr=0x%x,n_reg.val=0x%x\n");
	...
	case UREG_READ_I2C:
		...
		pr_err("youyoudebug UREG_READ_I2C == n_reg.paddr=0x%x,n_reg.vaddr=0x%x,n_reg.val=0x%x\n");
	...
	case UREG_WRITE_I2C:
		...
		pr_err("youyoudebug UREG_WRITE_I2C == n_reg.paddr=0x%x,n_reg.vaddr=0x%x,n_reg.val=0x%x\n");
