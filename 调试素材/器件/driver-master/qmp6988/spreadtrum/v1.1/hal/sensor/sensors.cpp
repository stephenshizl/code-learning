...
#include "sensors.h"
#if defined(FIS210X_SUPPORT)
#include "acc_gyro/fis210x.h"
#else
...
#endif
#if defined(FIS210X_SUPPORT)
				+GyroSensor::numSensors
#endif
#if defined(QMP6988_SUPPORT)
				+PressSensor::numSensors
#endif
                ] = {};
...
sensors_poll_context_t::sensors_poll_context_t() {
  ...
  ALOGD("numSensors=%d, AccSensor::numSensors=%d", numSensors, AccSensor::numSensors);
	...
	numSensors += mSensors[gyro]->populateSensorList(sSensorList + numSensors);
	mPollFds[gyro].fd = mSensors[gyro]->getFd();
	mPollFds[gyro].events = POLLIN;
	mPollFds[gyro].revents = 0;
	ALOGD("numSensors=%d, GyroSensor::numSensors=%d", numSensors, GyroSensor::numSensors);
#endif

#if defined(QMP6988_SUPPORT)
	mSensors[press] = new PressSensor();
	numSensors += mSensors[press]->populateSensorList(sSensorList + numSensors);
	mPollFds[press].fd = mSensors[press]->getFd();
	mPollFds[press].events = POLLIN;
	mPollFds[press].revents = 0;
	ALOGD("numSensors=%d, PressSensor::numSensors=%d", numSensors, PressSensor::numSensors);
#endif
