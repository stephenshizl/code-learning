...
static int64_t  previous_timestamp[OriSensor::__numSensors] = {0};
/*****************************************************************************/
static struct sensor_t sSensorList[] = {
	...
	{
		"AKM099XX Rotation vector sensor",
		"akm099xx",
		1,
	...
	mInputReader(32)
{
	...
	mPendingEvents[Rotationvetor].version = sizeof(sensors_event_t);
	mPendingEvents[Rotationvetor].sensor = ID_R;
	mPendingEvents[Rotationvetor].type = SENSOR_TYPE_ROTATION_VECTOR;
...
int OriSensor::setEnable(int32_t handle, int en)
{
	...
	if ((newState<<what) != (enabled & (1<<what))) {
		...
		int flags = newState;
		if( !newState && (mMaEnabled||mOrEnabled||mRvEnabled) ){
			err = 0;
		ALOGE("OriSensor (mMaEnabled||mOrEnabled||mRvEnabled)!=0,skip!\n");
		}else{
	...
	if (enabled == 0)
	{
		...
		{
			previous_timestamp[i] = 0;
		}
	}
	ALOGD("AccSensor: enabled = %d", enabled);
	return err;
...
int OriSensor::readEvents(sensors_event_t* data, int count)
{
	...
	while (count && mInputReader.readEvent(&event))
	{
		...
		if (type == EV_ABS) {
			...
			if(mMaEnabled || mOrEnabled || mRvEnabled)
			{
			...
			//		AKMDoCaliApi(time, mag_value, 0,calibmag, magbias, &lv);
					...
					mPendingEvents[MagneticField].magnetic.status = lv;
				}

				if(mOrEnabled || mRvEnabled)
				{
				...
				if(mOrEnabled)
				{
					...
					mPendingEvents[Orientation].orientation.azimuth =ori_value[0];
					mPendingEvents[Orientation].orientation.pitch = ori_value[1];
					mPendingEvents[Orientation].orientation.roll = ori_value[2];
					mPendingMask |= 1<<Orientation;
				}

				if(mRvEnabled)
				{
					GetRotationVector(rv_value);
					mPendingEvents[Rotationvetor].data[0] = rv_value[0];
					mPendingEvents[Rotationvetor].data[1] = rv_value[1];
					mPendingEvents[Rotationvetor].data[2] = rv_value[2];
					mPendingEvents[Rotationvetor].data[3] = rv_value[3];
					mPendingMask |= 1<<Rotationvetor;
				}
			...
			for (int j=0 ; count && mPendingMask && j<__numSensors ; j++) {
				if (mPendingMask & (1<<j)) {
					...
					if (enabled & (1<<j)) {
						if ((previous_timestamp[j]>0) && (mPendingEvents[j].timestamp>previous_timestamp[j]) && (mDelay[j]>0)) {
							int loop_cnt, i;
							int64_t temp_time;
							loop_cnt = (2*(mPendingEvents[j].timestamp-previous_timestamp[j]) + mDelay[j])/(2*mDelay[j]);
							previous_timestamp[j] = mPendingEvents[j].timestamp;
							if (loop_cnt > 0) {
